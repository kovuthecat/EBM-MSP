...
/**
 * Signature de ce qui est affiché pour un jeu de critères : options applicables (dans l'ordre trié par
 * le moteur) + alertes déclenchées. Deux jeux de critères ayant la même signature produisent le même
 * écran de décision. Recalcule les dérivés avant d'évaluer.
 */
function signature(node: Noeud, criteria: Criteria): string {
  ...
  const { applicable, alertes } = evaluateNode(node, derived)
  return `${applicable.map((o) => o.intitule).join('|')}##${alertes.map((a) => a.message).join('|')}`
}
