...
`soins-premiers` · `diabete-metabolisme` · `cardiovasculaire-prevention` (HTA, dyslipidémie, risque CV) · `bpco-pneumo` · `infectiologie-antibiotherapie` (durées de traitement, résistance, antibiothérapie courante) · `geriatrie-deprescription` · `pediatrie` (prescription et déprescription chez l'enfant, suivi du nourrisson) · `prevention-depistage-vaccination` · `ETP` · `sante-femme-perinatalite` · `orthophonie` · `soins-infirmiers` (plaies, observance, suivi) · `sante-mentale-addictologie` · `douleur-soins-palliatifs`.

**14 thèmes.** Liste identique à celle d'`ARCHITECTURE.md` § « Données affichées » → *Taxonomie de thèmes* — les deux doivent le rester. `infectiologie-antibiotherapie` a été ajouté le 2026-07-31 (D43) : §8bis le listait comme priorité MG et aucun des 12 thèmes précédents ne le couvrait. `pediatrie` a été ajouté le 2026-08-10 (**D63**) : la pédiatrie de soins premiers relève du champ du médecin généraliste, et aucun des 13 thèmes précédents ne la portait — un item pédiatrique devait jusque-là être rangé sous `soins-premiers`, ce qui le rendait introuvable au filtrage. Le périmètre de **production** de veille en couvre **12** (D40, amendé D60/D61/D63) ; les 2 autres (`ETP`, `soins-infirmiers`) restent au modèle de données, sans production.
