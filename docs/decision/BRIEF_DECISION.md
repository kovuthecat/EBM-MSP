...
> **Règles de modélisation, tous domaines : [`GRAMMAIRE-NOEUD.md`](GRAMMAIRE-NOEUD.md).** Le présent §5 décrit la **forme** du fichier ; la grammaire décrit **ce qu'il faut y mettre** pour que le nœud rende le raisonnement du praticien — un état ne se déduit pas d'une intention (R1), toute option porte son délai de bénéfice (R2), modifier un traitement est deux décisions (R3), écarté et non-indiqué sont deux silences distincts (R4), un critère qu'on demande doit agir (R5), l'argumentaire est situationnel (R6), le moteur ne se prononce jamais sur ce qu'il ignore (R7), un fait de sécurité a un canal et un seul (R8), un nœud qui recommande un geste sait si le geste est déjà fait (R9) — ainsi que la structure du banc de test d'un nœud. À lire **avant** d'écrire un nœud, dans quelque domaine que ce soit.

> **Procédé de construction, tous domaines : [`CONSTRUIRE-UN-MODULE.md`](CONSTRUIRE-UN-MODULE.md).** La grammaire dit *comment écrire* un nœud ; ce document dit *dans quel ordre le construire* — cadrage par la consultation avant la littérature, vignettes d'acceptation gelées **avant** le contenu, écran maquetté avant la collecte, double vérification fidélité / comportement — avec une porte de sortie par étape et des checklists opposables (critère, option, alerte, nœud, module). À ouvrir **au démarrage d'un domaine**, avant même le cadrage clinique.
